  ...
  getgenv().Danger = {
    ["Options"] = {
        ...
        ["Stealth Mode"] = {
            ...
            ["Key"] = "", -- // Enter your Danger Key
        },
        ["Internal"] = {
            ["Enabled"] = true, -- // Shows internal ui for Danger
...
--[[if Danger.Options["FPS Unlocker"].Enabled then 
    setfpscap(Danger.Options["FPS Unlocker"].FPS)
end]]

for i,v in pairs(game.Workspace:GetDescendants()) do 
    if v:IsA("Seat") then 
        v.Disabled = Danger["Extra"]["Disable Seats"]
    end
end
...
if not Danger["Extra"]["Low Graphics"] then
...
local Blacklisted_GameIds = loadstring(game:HttpGet("https://raw.githubusercontent.com/laagginq/Danger/main/blacklisted_gameIds.lua"))()

if table.find(Blacklisted_GameIds,game.PlaceId) then 
    game:GetService("StarterGui"):SetCore("SendNotification",{
        Title = "Blacklist game ID",
        Text = game:GetService("MarketplaceService"):GetProductInfo(game.PlaceId).Name.." is blacklisted because it is unsafe to use Danger on.", 
...
if Danger.Options.Version ~= utility.Version then 
    game:GetService("StarterGui"):SetCore("SendNotification",{
...
task.spawn(function()
    pcall(function()
        ...
        Memory:GetPropertyChangedSignal("Text"):Connect(function()
            if Danger.Spoofer.Memory.Enabled then 
                local random1 = math.random(Danger.Spoofer.Memory.Min, Danger.Spoofer.Memory.Max)
...
game:GetService("UserInputService").InputBegan:Connect(function(key, gp)
    if not gp then 
        if key.KeyCode == Danger.Emotes.LayKey then 
            if Danger.Emotes.Lay then 
        ...
        if key.KeyCode == Danger.Emotes.GreetKey then
            if Danger.Emotes.Greet then 
        ...
        if key.KeyCode == Danger.Emotes.SpeedKey then
            if Danger.Emotes.Speed then 
        ...
        if key.KeyCode == Danger.Emotes.SturdyKey then
            if Danger.Emotes.Sturdy then 
        ...
        if key.KeyCode == Danger.Emotes.GriddyKey then
            if Danger.Emotes.Griddy then 
...
local AkaliNotif = loadstring(game:HttpGet("https://raw.githubusercontent.com/laagginq/Danger/main/akali.lua"))();
...
getgenv().notify = function(txt) 
    if Danger.Options["Allow Notifications"] then
        if string.lower(Danger.Options["Notifcation Type"]) == "akali" then 
            gggg({
                Description = txt;
                Title = "Notification";
                Duration = Danger.Options["Notifcation Duration"];
            });
        elseif string.lower(Danger.Options["Notifcation Type"]) == "roblox" then
            game:GetService("StarterGui"):SetCore("SendNotification",{
                Title = "Notification",
                Text = txt, 
                Icon = "rbxassetid://14270853092",
        ...
        elseif string.lower(Danger.Options["Notifcation Type"]) == "xaxa" then
...
local closestsilentbodypart = Danger["Silent Aim"]["Target Part"]
local closestcamlockpart = Danger["Cam Lock"]["Target Part"]
...
Circle.Transparency = Danger.FOV["Silent Aim"].Transparency
Circle.Radius = Danger.FOV["Silent Aim"].Size * 3
Circle.Visible = Danger.FOV["Silent Aim"].Visible
Circle.Color = Danger.FOV["Silent Aim"].Color
Circle.Thickness = Danger.FOV["Silent Aim"].Thickness
Circle.Filled = Danger.FOV["Silent Aim"].Filled

local Circle2 = Drawing.new("Circle")
Circle2.Transparency = Danger.FOV["Cam Lock"].Transparency
Circle2.Radius = Danger.FOV["Cam Lock"].Size * 3
Circle2.Visible = Danger.FOV["Cam Lock"].Visible
Circle2.Color = Danger.FOV["Cam Lock"].Color
Circle2.Thickness = Danger.FOV["Cam Lock"].Thickness
Circle2.Filled = Danger.FOV["Cam Lock"].Filled
...
UpdateESP = function()
    for i,v in pairs(EspPlayers) do
        if Danger.ESP.Enabled and i ~= LocalPlayer and i.Character and i.Character:FindFirstChild("Humanoid") and i.Character:FindFirstChild("HumanoidRootPart") and i.Character:FindFirstChild("Head") then
            ...
            if OnScreen and GetDistance(i) < Danger.ESP["Max Distance"] then
                if Danger.ESP.Name.Enabled then
                    v.Name.Position = Vector2.new(BoxSize.X / 2 + BoxPos.X, BoxPos.Y - 16)
                    v.Name.Outline = Danger.ESP.Name.Outline
                    v.Name.Text = i.DisplayName.." (@"..i.Name..")"
                    v.Name.Color = Danger.ESP.Name.Color
                ...
                if Danger.ESP.Distance.Enabled and LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("HumanoidRootPart") then
                    v.Distance.Position = Vector2.new(BoxSize.X / 2 + BoxPos.X, BottomOffset)
                    v.Distance.Outline = Danger.ESP.Distance.Outline
                    v.Distance.Text = math.floor((Hrp.Position - LocalPlayer.Character.HumanoidRootPart.Position).Magnitude) .. " studs"
                    v.Distance.Color = Danger.ESP.Distance.Color
                ...
                if Danger.ESP.Box.Enabled then
                    v.BoxOutline.Size = BoxSize
                    v.BoxOutline.Position = BoxPos
                    v.BoxOutline.Visible = Danger.ESP.Box.Outline
                    v.BoxOutline.Color = Color3.fromRGB(0, 0, 0)
    
                    v.Box.Size = BoxSize
                    v.Box.Position = BoxPos
                    v.Box.Color = Danger.ESP.Box.Color
                    v.Box.Visible = true
                else
                    v.BoxOutline.Visible = false
                    v.Box.Visible = false
                end
                if Danger.ESP["Health Bar"].Enabled then
                    ...
                    v.HealthBar.Color = Danger.ESP["Health Bar"].Color
                    ...
                    v.HealthBarOutline.Visible = Danger.ESP["Health Bar"].Outline
                else
...
GetPrediction = function() 
    if Danger["Global"]["Auto Prediction"].Enabled then 

        local PingStats = game:GetService("Stats").Network.ServerStatsItem["Data Ping"]:GetValueString()
        local Value = tostring(PingStats)
        local PingValue = Value:split(" ")
        local PingNumber = tonumber(PingValue[1])
        if string.lower(Danger.Global["Auto Prediction"]["Version"]) == "default" then 
            return tonumber(PingNumber / 225 * 0.1 + 0.1)
        elseif string.lower(Danger.Global["Auto Prediction"]["Version"]) == "old" then 
            if PingNumber < 130 then
                return tonumber(PingNumber / 1000 + 0.037)
            else
                return tonumber(PingNumber / 1000 + 0.033)
            end
        elseif string.lower(Danger.Global["Auto Prediction"]["Version"]) == "v2" then 
        ...
        elseif string.lower(Danger.Global["Auto Prediction"]["Version"]) == "custom" then 
            if PingNumber < 30 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping20)
            elseif PingNumber < 40 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping30)
            elseif PingNumber < 50 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping40)
            elseif PingNumber < 60 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping50)
            elseif PingNumber < 70 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping60)
            elseif PingNumber < 80 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping70)
            elseif PingNumber < 90 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping80)
            elseif PingNumber < 100 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping90)
            elseif PingNumber < 110 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping100)
            elseif PingNumber < 120 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping110)
            elseif PingNumber < 130 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping120)
            elseif PingNumber < 140 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping130)
            elseif PingNumber < 150 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping140)
            elseif PingNumber < 160 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping150)
            elseif PingNumber < 170 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping160)
            elseif PingNumber < 180 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].P1ing70)
            elseif PingNumber < 190 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping180)
            elseif PingNumber < 200 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping190)
            elseif PingNumber < 210 then
                return tonumber(Danger.Global["Auto Prediction"]["Custom Settings"].Ping200)
            end
        end
    else
        return tonumber(Danger["Silent Aim"].Prediction)
    end
end
...
CheckDistance = function(Player)
    if (Player.Character.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude > Danger.Checks.Distance["Max Distance"] then 
        return true
    else
        return false
    end
...
GetClosestBodyPart = function()
    ...
    if Danger["Silent Aim"]["Nearest Part To Cursor"] then 
        closestsilentbodypart = tostring(BodyPart)
    else
        closestsilentbodypart = Danger["Silent Aim"]["Target Part"]
    end
...
GetClosestBodyPart2 = function()
    ...
    if Danger["Cam Lock"]["Nearest Part To Cursor"] then 
        closestcamlockpart = tostring(BodyPart)
    else
        closestcamlockpart = Danger["Cam Lock"]["Target Part"] 
    end
end
...
GetClosestPlayer = function()
    ...
    local HitChance = CalculateChance(Danger["Silent Aim"]["Hit Chance"])
    ...
    for _, v in pairs(Players:GetPlayers()) do
        if v.Character and v ~= LocalPlayer and v.Character:FindFirstChild("HumanoidRootPart") then
            ...
            if Danger.Checks["Visible Check"] and not RayCastCheck(v.Character.HumanoidRootPart, v.Character) then 
                continue 
            end
            if Danger.Checks["Whitelist"].Enabled and not table.find(Danger.Checks.Whitelist.Usernames,v.Name) then 
                continue 
            end
            if Danger.Checks["Blacklist"].Enabled and table.find(Danger.Checks.Blacklist.Usernames,v.Name) then 
                continue 
            end
            if table.find(safeplayers,v.UserId) then 
                continue 
            end
            if Danger.Checks["K.O Check"] and v.Character:FindFirstChild("BodyEffects") then
                local KoCheck 
            ...
            if Danger.Checks["Crew Check"] and FindCrew(v) and v.DataFolder.Information:FindFirstChild("Crew").Value == LocalPlayer.DataFolder.Information:FindFirstChild("Crew").Value then
                continue
            end
            if Danger.Checks["Friend Check"] and game.Players.LocalPlayer:IsFriendsWith(v.UserId) then
                continue
            end

            if Danger.Checks["Distance"].Enabled and CheckDistance(v) then
                continue
            end
...
game:GetService("RunService").RenderStepped:Connect(function()
    if Danger["Silent Aim"].Enabled and Danger["Silent Aim"]["Closest Point"] and SilentTarget ~= nil and SilentTarget.Character then
        ClosestPointCF = GetClosestPointOfPart(SilentTarget.Character[closestsilentbodypart])
    end
end)

game:GetService("RunService").RenderStepped:Connect(function()
    if Danger["Cam Lock"].Enabled and Danger["Cam Lock"]["Closest Point"] and CamlockTarget ~= nil and CamlockTarget.Character then
...
function SortInventory() 
     ...
     for x=1,#Danger["Gun Sorting"].Slots do
         for i=1,#temp do
             if string.sub(string.lower(tostring(temp[i])),1,string.len(string.lower(Danger["Gun Sorting"].Slots[x]))) == string.lower(Danger["Gun Sorting"].Slots[x]) then
                 temp[i].Parent = variables.LocalPlayer.Backpack
                 wait(0.105)
             end
         end
     end
     if Danger["Gun Sorting"]["Delete Extra"] then 
        for i=1,#temp do
            if not table.find(Danger["Gun Sorting"].Slots,temp[i].Name) then 
                temp[i]:Destroy()
                wait(0)
            end
        end
...
mouse.KeyDown:Connect(function(Key)
    local Keybind = Danger["Gun Sorting"].Bind:lower()
    if (Key == Keybind) then
        if Danger["Gun Sorting"].Enabled then 
            SortInventory()
        end
    end
end)
...
RunService.Heartbeat:Connect(function()
    UpdateFOV()
    if Danger["Silent Aim"].Enabled then 
        if string.lower(Danger["Silent Aim"].Mode) == "fov" then 
            GetClosestPlayer()
        end
    ...
    if Danger["Cam Lock"].Enabled then 
        if string.lower(Danger["Cam Lock"].Mode) == "fov" then 
            GetClosestPlayer2()
        end
...
mouse.KeyDown:Connect(function(Key)
    local Keybind = Danger["Silent Aim"]["Target Mode Bind"]:lower()
    local Keybind2 = Danger["Cam Lock"]["Target Mode Bind"]:lower()
    if (Key == Keybind) and Danger["Silent Aim"].Enabled then
        if string.lower(Danger["Silent Aim"].Mode) ==  "target" then 
            if SilentTarget == nil then 
                GetClosestPlayer()
                if Danger["Silent Aim"].Notify then 
                    if SilentTarget ~= nil then 
    ...
    if (Key == Keybind2 and Danger["Cam Lock"].Enabled) then
        if string.lower(Danger["Cam Lock"].Mode) ==  "target_toggle" then 
            if CamlockTarget == nil then 
                GetClosestPlayer2()
                if Danger["Cam Lock"].Notify then 
                    if CamlockTarget ~= nil then 
...
mouse.KeyDown:Connect(function(Key)
    local Keybind2 = Danger["Cam Lock"]["Target Mode Bind"]:lower()
    if (Key == Keybind2 and Danger["Cam Lock"].Enabled) then
        if string.lower(Danger["Cam Lock"].Mode) ==  "target_hold" then 
            if CamlockTarget == nil then 
                GetClosestPlayer2()
                if Danger["Cam Lock"].Notify then 
                    if CamlockTarget ~= nil then 
...
mouse.KeyDown:Connect(function(Key)
    if Key == (string.lower(Danger.Macro.Bind)) and Danger.Macro.Abuse == false then
        if Danger.Macro.Enabled then
            speeding = not speeding
            if speeding == true then
                repeat task.wait(Danger.Macro.Speed / 100)
                game:GetService("VirtualInputManager"):SendKeyEvent(true, "I", false, game)
                task.wait(Danger.Macro.Speed / 100)
...
mouse.KeyDown:Connect(function(Key)
    if Key == (string.lower(Danger.Macro.Bind)) and Danger.Macro.Abuse == true then
        if Danger.Macro.Enabled  then
            speeding = not speeding
            if speeding == true then
                repeat task.wait(Danger.Macro.Speed / 100)
...
mouse.KeyDown:Connect(function(Key)
    if Key == (string.lower(Danger["Fake Spike"].Bind))then
        if Danger["Fake Spike"].Enabled then
            settings():GetService("NetworkSettings").IncomingReplicationLag = 99999
            wait(Danger["Fake Spike"].Duration)
...
spawn(function()
    while wait(0.5) do 
        if Danger["Extra"]["Mute Boomboxes"] == true then 
...
mouse.KeyDown:Connect(function(Key)
    if Danger["Panic"].Enabled then
        if Key == (string.lower(Danger["Panic"].Bind))then
            if Danger["Panic"]["Things To Disable"].Options then
                Danger["Options"].Stats = false
                Danger["Options"]["Allow Notifications"] = false
            end

            if Danger["Panic"]["Things To Disable"].Aiming then
                Danger["Silent Aim"].Enabled = false
                Danger["Cam Lock"].Enabled = false
            end

            if Danger["Panic"]["Things To Disable"].FOV then
                Danger.FOV["Silent Aim"].Visible = false
                Danger.FOV["Cam Lock"].Visible = false
            end

            if Danger["Panic"]["Things To Disable"].Misc then
                Danger["Macro"].Enabled = false
                Danger["Gun Sorting"].Enabled = false
                Danger["Fake Spike"].Enabled = false
                Danger["Chat Commands"].Enabled = false
                Danger["Hit Sound"].Enabled = false
                Danger["Hit Log"].Enabled = false
            end

            if Danger["Panic"]["Things To Disable"].Emotes then
                Danger["Emotes"].Lay = false
                Danger["Emotes"].Greet = false
                Danger["Emotes"].Speed = false
                Danger["Emotes"].Sturdy = false
                Danger["Emotes"].Griddy = false
            end
        end
    end
end)
...
mouse.KeyDown:Connect(function(Key)
    local Keybind = Danger.Resolver.Keybind:lower()
    if (Key == Keybind) then
        if Danger.Resolver["Keybind Enabled"] then 
            if Danger.Resolver.Enabled == true then 
                Danger.Resolver.Enabled = false
                notify("Resolver disabled.")
            else
                Danger.Resolver.Enabled = true
...
RunService.Heartbeat:Connect(function()
    if Danger.Resolver.Enabled and Danger["Silent Aim"].Enabled then 
        if Danger.Resolver.Method == "Delta" then 
            if SilentTarget ~= nil then 
    ...
    if SilentTarget ~= nil then 
        ...
        if Danger["Silent Aim"]["Closest Point"] then
        ...
        TargetCF.Position + v3.new(0,Danger["Silent Aim"]["Target Part Offset"],0) + 
            (SilentTarget.Character[closestsilentbodypart].AssemblyLinearVelocity *
            Danger["Silent Aim"].Prediction))


    end
    if Danger["Silent Aim"].Dot and SilentTarget ~= nil and pos then
    ...
    if Danger["Silent Aim"].Line and SilentTarget ~= nil and pos then
...
RunService.Heartbeat:Connect(function()
    if Danger.Resolver.Enabled and Danger["Cam Lock"].Enabled then 
        if Danger.Resolver.Method == "Delta" then 
            if CamlockTarget ~= nil then 
    ...
    if CamlockTarget ~= nil then 
        ...
        if Danger["Cam Lock"]["Closest Point"] then
        ...
        pos = workspace.CurrentCamera:WorldToViewportPoint(
        TargetCF.Position + v3.new(0,Danger["Cam Lock"]["Target Part Offset"],0) + 
            (CamlockTarget.Character[closestcamlockpart].AssemblyLinearVelocity *
            Danger["Cam Lock"].Prediction))


    end
    if Danger["Cam Lock"].Dot and CamlockTarget ~= nil and pos then
    ...
    if Danger["Cam Lock"].Line and CamlockTarget ~= nil and pos then
...
--[[LPH_NO_VIRTUALIZE(function()
    ...
    oldIndex = hookmetamethod(game, "__index", function(self, Index, Check)
        ...
        if self == Mouseee and (Index:lower() == OldHit) then    
            if Danger["Silent Aim"].Enabled and Danger["Resolver"]["Aim Viewer Bypass"] == false then 
                if SilentTarget ~= nil then
                    ...
                    if Danger["Silent Aim"].Shake.Enabled then 
                        shake = v3.new(
                            math.random(-Danger["Silent Aim"].Shake.X, Danger["Silent Aim"].Shake.X),
                            math.random(-Danger["Silent Aim"].Shake.Y, Danger["Silent Aim"].Shake.Y),
                            math.random(-Danger["Silent Aim"].Shake.Z, Danger["Silent Aim"].Shake.Z)
                        ) * 0.1
                    end

                    if Danger.Airshot.Enabled then 
                        if SilentTarget.Character.Humanoid.FloorMaterial == Enum.Material.Air then 
                            offset = v3.new(0,Danger.Airshot["Jump Offset"],0)
                        end
                    end
                    ...
                    if Danger.Resolver.Enabled then 
                        if Danger.Resolver["Auto Detect"] then 
                    ...
                    if resolver then 
                        if Danger.Resolver.Method == "Move Direction" then 
                        ...
                        elseif Danger.Resolver.Method == "No Prediction" then 
                            targetbone.Velocity = v3.new(0,0,0)
                            targetbone.AssemblyLinearVelocity = v3.new(0,0,0)
                        elseif Danger.Resolver.Method == "Delta" or  Danger.Resolver.Method == "Recalculate" then 
                            targetbone.Velocity = newvel
                            targetbone.AssemblyLinearVelocity = newvel
                        end
                    end
                    if Danger["Silent Aim"]["Anti Ground Shots"] then 
                    ...
                    local Prediction = 
                    TargetCF + 
                    v3.new(0,Danger["Silent Aim"]["Target Part Offset"],0) + 
                    offset + 
                    targetbone.Velocity * 
        ...
        tool.Activated:Connect(function()
            if Danger["Silent Aim"].Enabled then 
                if SilentTarget ~= nil then
                    ...
                    if Danger["Global"]["Anti Ground Shots"] then 
...
LPH_NO_VIRTUALIZE(function()
    game:GetService("RunService").RenderStepped:Connect(function()
        if Danger["Global"]["Unlock Outside FOV"] and CamlockTarget and CamlockTarget.Character and CamlockTarget.Character:FindFirstChild(closestcamlockpart) then
            if Circle2.Radius <
                (Vector2.new(
        ...
        if Danger["Global"]["Unlock On KO"] == true and CamlockTarget ~= nil and CamlockTarget.Character and CamlockTarget.Character:FindFirstChild(closestcamlockpart) then
        ...
        if Danger["Global"]["Unlock Behind Wall"] == true and string.lower(Danger["Cam Lock"].Mode) ~= "fov" and CamlockTarget ~= nil and CamlockTarget.Character and CamlockTarget.Character:FindFirstChild(closestcamlockpart) then
        ...
        if Danger["Cam Lock"].Enabled and CamlockTarget and CamlockTarget.Character and CamlockTarget.Character:FindFirstChild(closestcamlockpart) then 
            if CamlockTarget ~= nil then
                ...
                if Danger["Cam Lock"].Shake.Enabled then 
                    shake = v3.new(
                        math.random(-Danger["Cam Lock"].Shake.X, Danger["Cam Lock"].Shake.X),
                        math.random(-Danger["Cam Lock"].Shake.Y, Danger["Cam Lock"].Shake.Y),
                        math.random(-Danger["Cam Lock"].Shake.Z, Danger["Cam Lock"].Shake.Z)
                    ) * 0.1
                end

                if Danger["Cam Lock"].Smoothness.Enabled then 
                    smoothing = Danger["Cam Lock"].Smoothness.Value
                end

                if Danger.Airshot.Enabled then 
                    if CamlockTarget.Character.Humanoid.FloorMaterial == Enum.Material.Air then 
                        offset = v3.new(0,Danger.Airshot["Jump Offset"],0)
                        if Danger.Airshot["Cam Lock Smoothness"].Enabled then 
                            smoothing = Danger.Airshot["Cam Lock Smoothness"]["New Smoothness"]
                        end
                    end
                end
                ...
                if resolver then 
                    ...
                    elseif Danger.Resolver.Method == "No Prediction" then 
                        targetbone.Velocity = v3.new(0,0,0)
                        targetbone.AssemblyLinearVelocity = v3.new(0,0,0)
                    elseif Danger.Resolver.Method == "Delta" then 
                        targetbone.Velocity = newvel
                        targetbone.AssemblyLinearVelocity = newvel
                    end
                end
                ...
                local Prediction = 
                TargetCF + 
                v3.new(0,Danger["Cam Lock"]["Target Part Offset"],0) + 
                offset + 
                targetbone.Velocity * 
                PredictionValue2 + shake
                local Main = CF.new(CurrentCamera.CFrame.p, Prediction)
                CurrentCamera.CFrame = CurrentCamera.CFrame:Lerp(Main, smoothing, Danger["Cam Lock"]["Easing Style"] , Enum.EasingDirection.InOut)
            end
        end
    end)
end)()
        ...
        connection = char:WaitForChild("Humanoid").HealthChanged:Connect(function(health)
            if health < oldhealth then 
                if plr == SilentTarget or plr == CamlockTarget then 
                    if Danger["Hit Sound"].Enabled then 
                        local Sound = Instance.new('Sound', CurrentCamera)
                        local SoundName = Danger["Hit Sound"]["Hit Sound"]
                        local SoundID = hitSounds[SoundName]
                        Sound.SoundId = SoundID
                        Sound.Volume = Danger["Hit Sound"].Volume
                        Sound:Play()
                        delay(10,function()
                            Sound:Destroy()
                        end)
                    end
                    if Danger["Hit Logs"].Enabled then 
...
game.Players.LocalPlayer.Chatted:Connect(function(msg)
    ...
    if Danger["Chat Commands"].Enabled then 
        if string.lower(text[1]) == string.lower(Danger["Chat Commands"].Prefix..Danger["Chat Commands"].Commands.Prediction) then 
            if tonumber(text[2]) then 
                Danger["Silent Aim"].Prediction = tonumber(text[2])
            else
                invalidargs(text[2])
            end
        end
        if string.lower(text[1]) == string.lower(Danger["Chat Commands"].Prefix..Danger["Chat Commands"].Commands.ToggleFOV) then 
            if string.lower(text[2]) == "true" or string.lower(text[2]) == "on" or string.lower(text[2]) == "yes" then 
                Danger.FOV["Silent Aim"].Visible = true
        ...
        if string.lower(text[1]) == string.lower(Danger["Chat Commands"].Prefix..Danger["Chat Commands"].Commands.SizeFOV) then 
            if tonumber(text[2]) then 
                Danger.FOV["Silent Aim"].Size = tonumber(text[2]) * 3
                Danger.FOV["Silent Aim"].Size = tonumber(text[2]) * 3
            else
                invalidargs(text[2])
            end
        end
        if string.lower(text[1]) == string.lower(Danger["Chat Commands"].Prefix..Danger["Chat Commands"].Commands.GotoLocation) then 
            if game.PlaceId == 2788229376 or game.PlaceId == 7213786345 then
        ...
        if string.lower(text[1]) == string.lower(Danger["Chat Commands"].Prefix..Danger["Chat Commands"].Commands.ToggleResolver) then 
            if string.lower(text[2]) == "true" or string.lower(text[2]) == "on" or string.lower(text[2]) == "yes" then 
                Danger.Resolver.DesyncResolver = true
            elseif string.lower(text[2]) == "false" or string.lower(text[2]) == "off" or string.lower(text[2]) == "no" then 
                Danger.Resolver.DesyncResolver = false
            else
                invalidargs(text[2])
            end
        end

        if string.lower(text[1]) == string.lower(Danger["Chat Commands"].Prefix..Danger["Chat Commands"].Commands.JoinDiscord) then 
            if httprequest then
                httprequest({
                    Url = 'http://127.0.0.1:6463/rpc?v=1',
                    Method = 'POST',
        ...
        if string.lower(text[1]) == string.lower(Danger["Chat Commands"].Prefix..Danger["Chat Commands"].Commands.RejoinServer) then 
            game:GetService("TeleportService"):Teleport(game.PlaceId, game.JobId, game:GetService("Players").LocalPlayer)
        end

        if string.lower(text[1]) == string.lower(Danger["Chat Commands"].Prefix..Danger["Chat Commands"].Commands.ToggleSilentAim) then 
            if string.lower(text[2]) == "true" or string.lower(text[2]) == "on" or string.lower(text[2]) == "yes" then 
                Danger["Silent Aim"].Enabled = true
            elseif string.lower(text[2]) == "false" or string.lower(text[2]) == "off" or string.lower(text[2]) == "no" then 
                Danger["Silent Aim"].Enabled = false
            else
                invalidargs(text[2])
        ...
        if string.lower(text[1]) == string.lower(Danger["Chat Commands"].Prefix..Danger["Chat Commands"].Commands.ToggleCamLock) then 
            if string.lower(text[2]) == "true" or string.lower(text[2]) == "on" or string.lower(text[2]) == "yes" then 
                Danger["Silent Aim"].Enabled = true
            elseif string.lower(text[2]) == "false" or string.lower(text[2]) == "off" or string.lower(text[2]) == "no" then 
                Danger["Silent Aim"].Enabled = false
        ...
        if string.lower(text[1]) == string.lower(Danger["Chat Commands"].Prefix..Danger["Chat Commands"].Commands.CamLockSmoothness) then 
            if tonumber(text[2]) then 
                Danger["Silent Aim"].Smoothness = tonumber(text[2])
            else
                invalidargs(text[2])
            end
        end

        if string.lower(text[1]) == string.lower(Danger["Chat Commands"].Prefix..Danger["Chat Commands"].Commands.CheckForExploiters) then 
            local found = false
            for i,v in pairs(game.Players:GetChildren()) do
        ...
        if string.lower(text[1]) == string.lower(Danger["Chat Commands"].Prefix..Danger["Chat Commands"].Commands.ForceReset) then 
            if game.PlaceId == 2788229376 or game.PlaceId == 7213786345 then
                ...
                game.Players.LocalPlayer.Character.Name = 'Danger ; #1'
            else
                unsupportedgame()
            end
        end
...
Mark.Text = "Danger v"..utility.Version.." | .gg/"..utility.Invite
...
game:GetService("RunService").Heartbeat:Connect(function()
    ...
    if Danger.Options.Stats then 
        Mark.Visible = true
        Pred.Visible = true
        Pred.Text = "Prediction: "..tostring(PredictionValue)
        Resolver.Visible = true
        Resolver.Text = "Resolver: "..tostring(Danger.Resolver.Enabled).." | "..Danger.Resolver.Method.." | {"..Danger.Resolver.Keybind.."}"
        Ping22.Visible = true
        Ping22.Text = "Ping: "..tostring(utility.Ping).."ms"
        silenttarg.Visible = true
        silenttarg.Text = "Silent Aim Target: "..tostring(SilentTarget).." | "..closestsilentbodypart.." | "..GetHealth(SilentTarget).." | "..GetDistance(SilentTarget).." studs | {"..Danger["Silent Aim"]["Target Mode Bind"].."}"
        camlocktarg.Visible = true
        camlocktarg.Text = "Camlock Target: "..tostring(CamlockTarget).." | "..closestcamlockpart.." | "..GetHealth(CamlockTarget).." | "..GetDistance(CamlockTarget).." studs | {"..Danger["Cam Lock"]["Target Mode Bind"].."}"
    else
        Mark.Visible = false
        Pred.Visible = false
...
if Danger["Options"]["Internal"].Enabled then 
    if Danger_UI_LOADED then 
        return
    else
        getgenv().Danger_UI_LOADED = true
        ...
        if string.find(LRM_UserNote,"dev") then 
            title = "Danger | Developer Build"
        end

        local Window = Library:CreateWindow({
            Title = title,
            Center = true,
            AutoShow = Danger["Options"]["Internal"]["Auto Show"],
            TabPadding = 8
        })
        ...
        SilentAimTab:AddToggle('SilentAim/Enabled', {
            Text = 'Enabled',
            Default = Danger["Silent Aim"].Enabled, 
            Tooltip = 'This will toggle the silent aim on Danger.', 
            Callback = function(Value)
                Danger["Silent Aim"].Enabled = Value
            end
        }):AddKeyPicker('SilentAim/TargetBind', {
            Default = Danger["Silent Aim"]["Target Mode Bind"],
            ...
            ChangedCallback = function(New)
                Danger["Silent Aim"]["Target Mode Bind"] = KeyCodeToString(New)
            end
        })
        
        SilentAimTab:AddToggle('SilentAim/Notify', {
            Text = 'Allow Notifications',
            Default = Danger["Silent Aim"].Notify, 
            Tooltip = 'This will toggle notifications for silent aim.', 
            Callback = function(Value)
                Danger["Silent Aim"].Notify = Value
            end
        })
        ...
        SilentAimTab:AddSlider('SilentAim/HitChance', {
            Text = 'Hit Chance',
            Default = Danger["Silent Aim"]["Hit Chance"],
            Min = 1,
            Max = 100,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["Silent Aim"]["Hit Chance"] = Value
            end
        })
        
        SilentAimTab:AddInput('SilentAim/Prediction', {
            Default = Danger["Silent Aim"].Prediction,
            ...
            Callback = function(Value)
                Danger["Silent Aim"].Prediction = tonumber(Value)
            end
        })
        ...
        SilentAimTab:AddDropdown('SilentAim/Mode', {
            Values = { 'FOV', 'Target' },
            Default = Danger["Silent Aim"]["Mode"], -- number index of the value / string
            ...
            Callback = function(Value)
                Danger["Silent Aim"]["Mode"] = Value
            end
        })
        
        SilentAimTab:AddDropdown('SilentAim/TargetPart', {
            Values = HitpartTable,
            Default = Danger["Silent Aim"]["Target Part"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Danger["Silent Aim"]["Target Part"] = Value
            end
        })
        
        SilentAimTab:AddSlider('SilentAim/TargetPartOffset', {
            Text = 'Offset',
            Default = Danger["Silent Aim"]["Target Part Offset"],
            Min = -5,
            Max = 5,
            Rounding = 1,
            Compact = true,
        ...
        SilentAimTab:AddToggle('SilentAim/Dot', {
            Text = 'Dot',
            Default = Danger["Silent Aim"]["Dot"], 
            Tooltip = 'Dot Prediction Visualizer', 
            Callback = function(Value)
                Danger["Silent Aim"]["Dot"] = Value
            end
        })
        
        SilentAimTab:AddToggle('SilentAim/Line', {
            Text = 'Line',
            Default = Danger["Silent Aim"]["Line"], 
            Tooltip = 'Line Prediction Visualizer', 
            Callback = function(Value)
                Danger["Silent Aim"]["Line"] = Value
            end
        })
        ...
        SilentAimTab:AddToggle('SilentAim/FOV/Visible', {
            Text = 'Show FOV',
            Default = Danger["FOV"]["Silent Aim"].Visible , 
            Tooltip = 'Shows FOV Circle', 
            Callback = function(Value)
                Danger["FOV"]["Silent Aim"].Visible = Value
            end
        }):AddColorPicker('SilentFOVColor', {
            Default = Danger["FOV"]["Silent Aim"].Color, -- Bright green
            ...
            Callback = function(Value)
                Danger["FOV"]["Silent Aim"].Color = Value
            end
        })
        ...
        SilentAimFOV:AddToggle('SilentAim/FOV/Filled', {
            Text = 'Filled',
            Default = Danger["FOV"]["Silent Aim"].Filled , 
            Tooltip = 'ewww', 
            Callback = function(Value)
                Danger["FOV"]["Silent Aim"].Filled = Value
            end
        })
        
        SilentAimFOV:AddSlider('SilentAim/FOV/Size', {
            Text = 'Size',
            Default = Danger["FOV"]["Silent Aim"].Size,
            Min = 1,
            Max = 100,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["FOV"]["Silent Aim"].Size = Value
            end
        })
        
        SilentAimFOV:AddSlider('SilentAim/FOV/Thickness', {
            Text = 'Thickness',
            Default = Danger["FOV"]["Silent Aim"].Thickness,
            Min = 1,
            Max = 10,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["FOV"]["Silent Aim"].Thickness = Value
            end
        })
        ...
        SilentAimFOV:AddSlider('SilentAim/FOV/Transparency', {
            Text = 'Transparency',
            Default = Danger["FOV"]["Silent Aim"].Transparency,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["FOV"]["Silent Aim"].Transparency = Value
            end
        })
        ...
        CamLockTab:AddToggle('CamLock/Enabled', {
            Text = 'Enabled',
            Default = Danger["Cam Lock"].Enabled, 
            Tooltip = 'This will toggle the Cam Lock on Danger.', 
            Callback = function(Value)
                Danger["Cam Lock"].Enabled = Value
            end
        }):AddKeyPicker('CamLock/TargetBind', {
            Default = Danger["Cam Lock"]["Target Mode Bind"],
            SyncToggleState = false,
            ...
            NoUI = false, 
            ChangedCallback = function(New)
                Danger["Cam Lock"]["Target Mode Bind"] = KeyCodeToString(New)
            end
        })
        
        CamLockTab:AddToggle('CamLock/Notify', {
            Text = 'Allow Notifications',
            Default = Danger["Cam Lock"].Notify, 
            Tooltip = 'This will toggle notifications for Cam Lock.', 
            Callback = function(Value)
                Danger["Cam Lock"].Notify = Value
            end
        })
        
        CamLockTab:AddInput('CamLock/Prediction', {
            Default = Danger["Cam Lock"].Prediction,
            Numeric = true, -- true / false, only allows numbers
            Finished = true, -- true / false, only calls callback when you press enter
            ...
            Callback = function(Value)
                Danger["Cam Lock"].Prediction = tonumber(Value)
            end
        })
        ...
        CamLockTab:AddDropdown('CamLock/Mode', {
            ...
            Default = Danger["Cam Lock"]["Mode"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Danger["Cam Lock"]["Mode"] = Value
            end
        })
        
        CamLockTab:AddDropdown('CamLock/EasingStyle', {
            Values = EasingStyles,
            Default = Danger["Cam Lock"]["Easing Style"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Danger["Cam Lock"]["Easing Style"] = Value
            end
        })
        
        CamLockTab:AddDropdown('CamLock/TargetPart', {
            Values = HitpartTable,
            Default = Danger["Cam Lock"]["Target Part"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Danger["Cam Lock"]["Target Part"] = Value
            end
        })
        
        CamLockTab:AddSlider('CamLock/TargetPartOffset', {
            Text = 'Offset',
            Default = Danger["Cam Lock"]["Target Part Offset"],
            Min = -5,
            Max = 5,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["Cam Lock"]["Target Part Offset"] = Value
            end
        })
        
        CamLockTab:AddToggle('CamLock/NPTC', {
            Text = 'Nearest Part To Cursor',
            Default = Danger["Cam Lock"]["Nearest Part To Cursor"], 
            Tooltip = 'This will get the nearest target part to your cursor.', 
            Callback = function(Value)
                Danger["Cam Lock"]["Nearest Part To Cursor"] = Value
            end
        })
        
        CamLockTab:AddToggle('CamLock/NP', {
            Text = 'Closest Point',
            Default = Danger["Cam Lock"]["Closest Point"], 
            Tooltip = 'This will get the nearest point to your cursor.', 
            Callback = function(Value)
                Danger["Cam Lock"]["Closest Point"] = Value
            end
        })
        ...
        CamLockTab:AddToggle('CamLock/Dot', {
            Text = 'Dot',
            Default = Danger["Cam Lock"]["Dot"], 
            Tooltip = 'Dot Prediction Visualizer', 
            Callback = function(Value)
                Danger["Cam Lock"]["Dot"] = Value
            end
        })
        
        CamLockTab:AddToggle('CamLock/Line', {
            Text = 'Line',
            Default = Danger["Cam Lock"]["Line"], 
            Tooltip = 'Line Prediction Visualizer', 
            Callback = function(Value)
                Danger["Cam Lock"]["Line"] = Value
            end
        })
        ...
        CamLockTab:AddToggle('CamLock/Smooth', {
            ...
            Default = Danger["Cam Lock"]["Smoothness"].Enabled , 
            Tooltip = 'Camera Smoothness', 
            Callback = function(Value)
                Danger["Cam Lock"]["Smoothness"].Enabled = Value
            end
        })
        ...
        SmootnessValue:AddSlider('CamLock/SmoothV', {
            Text = 'Value',
            Default = Danger["Cam Lock"]["Smoothness"].Value,
            Min = 0,
            Max = 1,
            Rounding = 10,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["Cam Lock"]["Smoothness"].Value = Value
            end
        })
        ...
        CamLockTab:AddToggle('CamLock/Shake', {
            Text = 'Enable Shake',
            Default = Danger["Cam Lock"]["Shake"].Enabled , 
            Tooltip = 'Camera Shake', 
            Callback = function(Value)
                Danger["Cam Lock"]["Shake"].Enabled = Value
            end
        })
        ...
        ValuesDEP:AddSlider('CamLock/ShakeX', {
            Text = 'X',
            Default = Danger["Cam Lock"]["Shake"].X,
            Min = 0,
            Max = 10,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["Cam Lock"]["Shake"].X = Value
            end
        })
        
        ValuesDEP:AddSlider('CamLock/ShakeY', {
            Text = 'Y',
            Default = Danger["Cam Lock"]["Shake"].Y,
            Min = 0,
            Max = 10,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["Cam Lock"]["Shake"].Y = Value
            end
        })
        
        ValuesDEP:AddSlider('CamLock/ShakeZ', {
            Text = 'Z',
            Default = Danger["Cam Lock"]["Shake"].Z,
            Min = 0,
            Max = 10,
            Rounding = 1,
            Compact = true,
        ...
        CamLockTab:AddToggle('CamLock/FOV/Visible', {
            Text = 'Show FOV',
            Default = Danger["FOV"]["Cam Lock"].Visible , 
            Tooltip = 'Shows FOV Circle', 
            Callback = function(Value)
                Danger["FOV"]["Cam Lock"].Visible = Value
            end
        }):AddColorPicker('CamlockFOVColor', {
            Default = Danger["FOV"]["Cam Lock"].Color, -- Bright green
            Title = 'Circle Color', -- Optional. Allows you to have a custom color picker title (when you open it)
            Transparency = nil, -- Optional. Enables transparency changing for this color picker (leave as nil to disable)
        
            Callback = function(Value)
                Danger["FOV"]["Cam Lock"].Color = Value
            end
        })
        ...
        CamLockFOV:AddToggle('CamLock/FOV/Filled', {
            Text = 'Filled',
            Default = Danger["FOV"]["Cam Lock"].Filled , 
            Tooltip = 'ewww', 
            Callback = function(Value)
                Danger["FOV"]["Cam Lock"].Filled = Value
            end
        })
        
        CamLockFOV:AddSlider('CamLock/FOV/Size', {
            Text = 'Size',
            Default = Danger["FOV"]["Cam Lock"].Size,
            Min = 1,
            Max = 100,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["FOV"]["Cam Lock"].Size = Value
            end
        })
        
        CamLockFOV:AddSlider('CamLock/FOV/Thickness', {
            Text = 'Thickness',
            Default = Danger["FOV"]["Cam Lock"].Thickness,
            Min = 1,
            Max = 10,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["FOV"]["Cam Lock"].Thickness = Value
            end
        })
        ...
        CamLockFOV:AddSlider('CamLock/FOV/Transparency', {
            Text = 'Transparency',
            Default = Danger["FOV"]["Cam Lock"].Transparency,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["FOV"]["Cam Lock"].Transparency = Value
            end
        })
        ...
        Global:AddToggle('Global/AutoPrediction', {
            ...
            Default = Danger["Global"]["Auto Prediction"].Enabled , 
            Tooltip = 'Uses ping based auto prediction.', 
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"].Enabled = Value
            end
        })
        ...
        Depbox:AddDropdown('Global/AutoPrediction/Version', {
            ...
            Default = Danger["Global"]["Auto Prediction"]["Version"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Version"] = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_20', {
            Text = '20',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping20,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping20 = Value
            end
        })
        CustomValues:AddSlider('ping_30', {
            Text = '30',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping30,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping30 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_40', {
            Text = '40',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping40,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping40 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_50', {
            Text = '50',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping50,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping50 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_60', {
            Text = '60',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping60,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping60 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_70', {
            Text = '70',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping70,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping70 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_80', {
            Text = '80',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping80,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping80 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_90', {
            Text = '90',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping90,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping90 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_100', {
            Text = '100',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping100,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping100 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_110', {
            Text = '110',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping110,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping110 = Value
            end
        })
        
        CustomValues:AddSlider('ping_120', {
            Text = '120',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping120,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping120 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_130', {
            Text = '130',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping130,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping130 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_140', {
            Text = '140',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping140,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping140 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_150', {
            Text = '150',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping150,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping150 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_160', {
            Text = '160',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping160,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping160 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_170', {
            Text = '170',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping170,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping170 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_180', {
            Text = '180',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping180,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping180 = Value
            end
        })
        
        CustomValues:AddSlider('ping_190', {
            Text = '190',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping190,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping190 = Value
            end
        })
        ...
        CustomValues:AddSlider('ping_200', {
            Text = '200',
            Default = Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping200,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Global"]["Auto Prediction"]["Custom Settings"].Ping200 = Value
            end
        })
        ...
        Global:AddToggle('Global/AntiGroundShots', {
            ...
            Default = Danger["Global"]["Anti Ground Shots"] , 
            Tooltip = 'Will prevent you from shooting at the ground.', 
            Callback = function(Value)
                Danger["Global"]["Anti Ground Shots"] = Value
            end
        })
        
        Global:AddToggle('Global/UnlockOnKO', {
            Text = 'Unlock On KO',
            Default = Danger["Global"]["Unlock On KO"] , 
            Tooltip = 'Unlocks when target is knocked.', 
            Callback = function(Value)
                Danger["Global"]["Unlock On KO"] = Value
            end
        })
        ...
        Checks:AddToggle('Checks/Whitelist', {
            Text = 'Whitelist',
            Default = Danger["Checks"]["Whitelist"].Enabled , 
            Tooltip = 'Will only lock onto people whitelisted', 
            Callback = function(Value)
                Danger["Checks"]["Whitelist"].Enabled = Value
            end
        })
        ...
        Whitelist:AddDropdown('Checks/Whitelist/SelectPlayer', {
            ...
            Callback = function(Value)
                Danger["Checks"]["Whitelist"].Usernames = Value
            end
        })
        ...
        Checks:AddToggle('Checks/Blacklist', {
            Text = 'Blacklist',
            Default = Danger["Checks"]["Blacklist"].Enabled , 
            Tooltip = 'Will not lock onto people blacklisted', 
            Callback = function(Value)
                Danger["Checks"]["Blacklist"].Enabled = Value
            end
        })
        ...
        Blacklist:AddDropdown('Checks/Blacklist/SelectPlayer', {
            ...
            Tooltip = 'Select Targets', -- Information shown when you hover over the dropdown
            Callback = function(Value)
                Danger["Checks"]["Blacklist"].Usernames = Value
            end
        })
        ...
        Checks:AddToggle('Checks/Distance', {
            Text = 'Distance',
            Default = Danger["Checks"]["Distance"].Enabled , 
            Tooltip = 'Will not lock onto people who are too far away', 
            Callback = function(Value)
                Danger["Checks"]["Distance"].Enabled = Value
            end
        })
        ...
        Distance:AddSlider('Checks/Distance/Max', {
            Text = 'Max Distance',
            Default = Danger["Checks"]["Distance"]["Max Distance"],
            Min = 1,
            Max = 1000,
            Rounding = 0,
            Compact = true, -- If set to true, then it will hide the label
            Callback = function(Value)
                Danger["Checks"]["Distance"]["Max Distance"] = Value
            end
        })
        ...
        Resolver:AddToggle('Resolver/Enabled', {
            Text = 'Enabled',
            Default = Danger["Resolver"].Enabled , 
            Tooltip = 'Will toggle resolver.', 
            Callback = function(Value)
                Danger["Resolver"].Enabled = Value
            end
        }):AddKeyPicker('Resolver/ToggleBind', {
            Default = Danger.Resolver.Keybind, 
        ...
        Resolver:AddDropdown('Resolver/Method', {
            ...
            Default = Danger["Resolver"]["Method"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
        ...
        Airshot:AddToggle('Airshot/Enabled', {
            Text = 'Enable Airshot',
            Default = Danger["Airshot"].Enabled , 
            Tooltip = 'Will change your target offset if player is in air', 
            Callback = function(Value)
                Danger["Airshot"].Enabled = Value
            end
        })
        ...
        ssss:AddSlider('Airshot/JumpOffset', {
            Text = 'Offset',
            Default = Danger["Airshot"]["Jump Offset"],
            Min = -5,
            Max = 5,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["Airshot"]["Jump Offset"] = Value
            end
        })
        ssss:AddDivider()
        ...
        Airshot:AddToggle('Airshot/CamlockSmoothness', {
            ...
            Default = Danger["Airshot"]["Cam Lock Smoothness"].Enabled , 
            Tooltip = 'Will change your camlock smoothness if target is in air', 
            Callback = function(Value)
                Danger["Airshot"]["Cam Lock Smoothness"].Enabled = Value
            end
        })
        ...
        SMoothing:AddSlider('CamLock/SmoothV2', {
            Text = 'Value',
            Default = Danger["Airshot"]["Cam Lock Smoothness"]["New Smoothness"],
            Min = 0,
            Max = 1,
            Rounding = 10,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["Airshot"]["Cam Lock Smoothness"]["New Smoothness"] = Value
            end
        })
        ...
        Visuals:AddToggle('HitSound/Enabled', {
            Text = 'Hit Sound',
            Default = Danger["Hit Sound"].Enabled , 
            Tooltip = 'Will play sound when you hit target', 
            Callback = function(Value)
                Danger["Hit Sound"].Enabled = Value
            end
        })
        ...
        HitSound:AddSlider('HitSound/Volume', {
            Text = 'Volume',
            Default = Danger["Hit Sound"].Volume * 10,
            Min = 1,
            Max = 100,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["Hit Sound"].Volume = Value / 10
            end
        })
        ...
        HitSound:AddDropdown('HitSound/Sound', {
            ...
            Default = Danger["Hit Sound"]["Hit Sound"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Danger["Hit Sound"]["Hit Sound"] = Value
            end
        })
        ...
        Visuals:AddToggle('HitLogs/Enabled', {
            Text = 'Hit Logs',
            Default = Danger["Hit Logs"].Enabled , 
            Tooltip = 'Logs when you hit a player', 
            Callback = function(Value)
                Danger["Hit Logs"].Enabled = Value
            end
        })
        ...
        Misc:AddToggle('Macro/Enabled', {
            Text = 'Macro',
            Default = Danger["Macro"].Enabled , 
            Tooltip = 'Lua based Macro', 
            Callback = function(Value)
                Danger["Macro"].Enabled = Value
            end
        }):AddKeyPicker('Macro/Bind', {
            Default = Danger["Macro"]["Bind"],
            SyncToggleState = false,
            ...
            NoUI = false, 
            ChangedCallback = function(New)
                Danger["Macro"]["Bind"] = KeyCodeToString(New)
            end
        })
        ...
        MacroSets:AddToggle('Macro/Abuse', {
            Text = 'Abuse',
            Default = Danger["Macro"].Abuse , 
            Tooltip = 'Use scroll wheel to macro', 
            Callback = function(Value)
                Danger["Macro"].Abuse = Value
            end
        })

        MacroSets:AddSlider('Macro/Speed', {
            Text = 'Speed',
            Default = Danger["Macro"].Speed,
            Min = 0,
            Max = 1,
            Rounding = 3,
            Compact = true,
        ...
        Misc:AddToggle('MemorySpoofer/Enabled', {
            Text = 'Memory Spoofer',
            Default = Danger["Spoofer"].Memory.Enabled , 
            Tooltip = 'Spoofs your memory', 
            Callback = function(Value)
                Danger["Spoofer"].Memory.Enabled = Value
            end
        })
        ...
        SpooferSets:AddSlider('MemorySpoofer/Min', {
            Text = 'Minimum',
            Default = Danger["Spoofer"].Memory.Min,
            Min = 500,
            Max = 2000,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["Spoofer"].Memory.Min = Value
            end
        })

        SpooferSets:AddSlider('MemorySpoofer/Max', {
            Text = 'Maximum',
            Default = Danger["Spoofer"].Memory.Max,
            Min = 500,
            Max = 2000,
            Rounding = 1,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["Spoofer"].Memory.Max = Value
            end
        })
        ...
        Misc:AddToggle('FakeSpike/Enabled', {
            Text = 'Fake Spike',
            Default = Danger["Fake Spike"].Enabled , 
            Tooltip = 'Spikes your ping to make it look like your lagging', 
            Callback = function(Value)
                Danger["Fake Spike"].Enabled = Value
            end
        }):AddKeyPicker('FakeSpike/Bind', {
            Default = Danger["Fake Spike"]["Bind"],
            SyncToggleState = false,
            ...
            NoUI = false, 
            ChangedCallback = function(New)
                Danger["Fake Spike"]["Bind"] = KeyCodeToString(New)
            end
        })
        ...
        Spike:AddSlider('FakeSpike/Duration', {
            Text = 'Duration',
            Default =  Danger["Fake Spike"]["Duration"],
            Min = 0.1,
            Max = 3,
            Rounding = 2,
            Compact = true,
            ...
            Callback = function(Value)
                Danger["Fake Spike"]["Duration"] = Value
            end
        })
        ...
        Misc:AddToggle('Panic/Enabled', {
            Text = 'Panic',
            Default = Danger["Panic"].Enabled , 
            Tooltip = 'Will disable everything when your press keybind', 
            Callback = function(Value)
                Danger["Panic"].Enabled = Value
            end
        }):AddKeyPicker('Panic/Bind', {
            Default = Danger["Panic"]["Bind"],
            SyncToggleState = false,
            ...
            NoUI = false, 
            ChangedCallback = function(New)
                Danger["Panic"]["Bind"] = KeyCodeToString(New)
            end
        })
        ...
        PanicMode:AddToggle('Panic/Options', {
            Text = 'Options',
            Default = Danger["Panic"]["Things To Disable"].Options , 
            Callback = function(Value)
                Danger["Panic"]["Things To Disable"].Options = Value
            end
        })

        PanicMode:AddToggle('Panic/Aiming', {
            Text = 'Aiming',
            Default = Danger["Panic"]["Things To Disable"].Aiming , 
            Callback = function(Value)
                Danger["Panic"]["Things To Disable"].Aiming = Value
            end
        })
        ...
        PanicMode:AddToggle('Panic/FOV', {
            Text = 'FOV',
            Default = Danger["Panic"]["Things To Disable"].FOV , 
            Callback = function(Value)
                Danger["Panic"]["Things To Disable"].FOV = Value
            end
        })

        PanicMode:AddToggle('Panic/Misc', {
            Text = 'Misc',
            Default = Danger["Panic"]["Things To Disable"].Misc , 
            Callback = function(Value)
                Danger["Panic"]["Things To Disable"].Misc = Value
            end
        })
        ...
        PanicMode:AddToggle('Panic/Emotes', {
            Text = 'Emotes',
            Default = Danger["Panic"]["Things To Disable"].Emotes , 
            Callback = function(Value)
                Danger["Panic"]["Things To Disable"].Emotes = Value
            end
        })
        ...
        Misc:AddToggle('GunSorting/Enabled', {
            Text = 'Gun Sorting',
            Default = Danger["Gun Sorting"].Enabled , 
            Tooltip = 'Sorts your guns in your hotbar (must set in table)', 
            Callback = function(Value)
                Danger["Gun Sorting"].Enabled = Value
            end
        }):AddKeyPicker('GunSorting/Bind', {
            Default = Danger["Gun Sorting"]["Bind"],
            SyncToggleState = false,
            ...
            NoUI = false, 
            ChangedCallback = function(New)
                Danger["Gun Sorting"]["Bind"] = KeyCodeToString(New)
            end
        })
        ...
        Sort:AddToggle('GunSorting/Delete', {
            Text = 'Delete Extra',
            Default = Danger["Gun Sorting"]["Delete Extra"] , 
            Tooltip = 'Deletes unsorted items from your inventory', 
            Callback = function(Value)
                Danger["Gun Sorting"]["Delete Extra"] = Value
            end
        })
        ...
        ESP:AddToggle('Esp/Enabled', {
            Text = 'Enabled',
            Default = Danger["ESP"].Enabled , 
            Callback = function(Value)
                Danger["ESP"].Enabled = Value
            end
        })

        ESP:AddSlider('ESP/MAXDISTANCE', {
            Text = 'Max Distance',
            Default = Danger.ESP["Max Distance"],
            Min = 1,
            Max = 1000,
            Rounding = 0,
            Compact = true,
            ...
            Callback = function(Value)
                Danger.ESP["Max Distance"] = Value
            end
        })

        ESP:AddToggle('Esp/Names', {
            Text = 'Names',
            Default = Danger["ESP"].Name.Enabled , 
            Callback = function(Value)
                Danger["ESP"].Name.Enabled = Value
            end
        }):AddColorPicker('NameColor', {
            Default = Danger["ESP"].Name.Color,
            Title = 'Name Color', 
            Transparency = nil, 
        
            Callback = function(Value)
                Danger["ESP"].Name.Color = Value
            end
        })

        ESP:AddToggle('Esp/Names/Outline', {
            Text = 'Name Outline',
            Default = Danger["ESP"].Name.Outline , 
            Callback = function(Value)
                Danger["ESP"].Name.Outline = Value
            end
        })
        ...
        ESP:AddToggle('Esp/Box', {
            Text = 'Box',
            Default = Danger["ESP"].Box.Enabled , 
            Callback = function(Value)
                Danger["ESP"].Box.Enabled = Value
            end
        }):AddColorPicker('BoxColor', {
            Default = Danger["ESP"].Box.Color,
            Title = 'Box Color', 
            Transparency = nil, 
        
            Callback = function(Value)
                Danger["ESP"].Box.Color = Value
            end
        })

        ESP:AddToggle('Esp/Names/Outline', {
            Text = 'Box Outline',
            Default = Danger["ESP"].Box.Outline , 
            Callback = function(Value)
                Danger["ESP"].Box.Outline = Value
            end
        })
        ...
        ESP:AddToggle('Esp/Health', {
            Text = 'Health Bar',
            Default = Danger["ESP"]["Health Bar"].Enabled , 
            Callback = function(Value)
                Danger["ESP"]["Health Bar"].Enabled = Value
            end
        }):AddColorPicker('HealthColor', {
            Default = Danger["ESP"]["Health Bar"].Color,
            Title = 'Health Bar Color', 
            Transparency = nil, 
        
            Callback = function(Value)
                Danger["ESP"]["Health Bar"].Color = Value
            end
        })
        ...
        ESP:AddToggle('Esp/Health/Outline', {
            ...
            Default = Danger["ESP"]["Health Bar"].Outline , 
            Callback = function(Value)
                Danger["ESP"]["Health Bar"].Outline = Value
            end
        })

        ESP:AddToggle('Esp/Distance', {
            Text = 'Distance',
            Default = Danger["ESP"].Distance.Enabled , 
            Callback = function(Value)
                Danger["ESP"].Distance.Enabled = Value
            end
        }):AddColorPicker('DistanceColor', {
            Default = Danger["ESP"].Distance.Color,
            Title = 'Distance Color', 
            Transparency = nil, 
        
            Callback = function(Value)
                Danger["ESP"].Distance.Color = Value
            end
        })

        ESP:AddToggle('Esp/Distance/Outline', {
            Text = 'Distance Outline',
            Default = Danger["ESP"].Distance.Outline , 
            Callback = function(Value)
                Danger["ESP"].Distance.Outline = Value
            end
        })
        ...
        -- UI Settings
        Library:SetWatermark('Danger | '..tostring(os.date("%B") .. " " .. os.date("%d") .. " " .. os.date("%Y")).." | "..utility.Version)
        Library:SetWatermarkVisibility(false)
        local MenuGroup = Tabs['UI Settings']:AddLeftGroupbox('Menu')
        local MenuGroup2 = Tabs['UI Settings']:AddRightGroupbox('More Settings')
        MenuGroup:AddButton('Unload', function() Library:Unload() end)
        MenuGroup:AddLabel('Menu bind'):AddKeyPicker('MenuKeybind', { Default = Danger["Options"]["Internal"].Bind, NoUI = true, Text = 'Menu keybind' })
        ...
        MenuGroup2:AddToggle('show_stats', {
            Text = 'Show Stats',
            Default = Danger["Options"]["Stats"],
            Callback = function(V) 
                Danger["Options"]["Stats"] = V
            end
        })
        
        MenuGroup2:AddToggle('allow_notifications', {
            Text = 'Allow Notifications',
            Default = Danger["Options"]["Allow Notifications"],
            Callback = function(V) 
                Danger["Options"]["Allow Notifications"] = V
            end
        })

        MenuGroup2:AddDropdown('notification_mode', {
            Values = NotificationTypes,
            Default = Danger.Options["Notifcation Type"], -- number index of the value / string
            Multi = false, -- true / false, allows multiple choices to be selected
            ...
            Callback = function(Value)
                Danger.Options["Notifcation Type"] = Value
            end
        })
        ...
        if game:GetService("UserInputService").TouchEnabled then 
            task.spawn(function()
                local DangerMobileButton = Instance.new("ScreenGui")
                local Icon = Instance.new("Frame")
                local IconButton = Instance.new("TextButton")
                local IconImage = Instance.new("ImageLabel")
                local IconCorner = Instance.new("UICorner")
                DangerMobileButton.Name = "DangerMobileButton"
                DangerMobileButton.IgnoreGuiInset = true
                DangerMobileButton.Parent = game.CoreGui
                DangerMobileButton.DisplayOrder = 10
                DangerMobileButton.ResetOnSpawn = false
                Icon.Name = "Icon"
                Icon.Parent = DangerMobileButton
                ...
                IconButton.MouseButton1Down:Connect(function()
                    game:GetService("VirtualInputManager"):SendKeyEvent(true, string.upper(Danger["Options"]["Internal UI Bind"]) , false , game)
                end)
            end)
        end
    end
